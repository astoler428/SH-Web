...
export default function PolicyPiles({game, boardDimensions}) {
  ...
  const policyPilesWidth = boardDimensions.x/9
  const policyPilesBorderRadius = policyPilesWidth/18
  const countStyles = {
    position: 'absolute',
    top: 0,
    left: 0, color:
  ...
  return (
      <Box sx={{display: 'flex', gap: 1}}>
        <Box sx={{position: 'absolute', left: boardDimensions.x/34, bottom: boardDimensions.x/40, width: policyPilesWidth}}>
          <img src={PolicyBack} style={{ width: '100%', borderRadius: policyPilesBorderRadius }}/>
          <Box sx={countStyles}>
            {game.deck.drawPile.length}
          </Box>
        </Box>
        <Box sx={{position: 'absolute', right: boardDimensions.x/44, bottom: boardDimensions.x/40, width: policyPilesWidth}}>
          <img src={PolicyBack} style={{ width: '100%', borderRadius: policyPilesBorderRadius }}/>
          <Box sx={countStyles}>
